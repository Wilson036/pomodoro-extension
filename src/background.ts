    ...
    chrome.storage.local.get(
      ...
      (result: StorageData) => {
        if (result.isRunning && result.startTime) {
          ...
          const newTimeLeft = Math.max(0, (result.timeLeft || 0) - elapsed);

          if (newTimeLeft === 0) {
            // 時間到了，發送通知
            chrome.storage.local.get(["isWorking"], (data: StorageData) => {
...
chrome.runtime.onInstalled.addListener(() => {
  ...
  // 檢查是否有正在運行的計時器
  chrome.storage.local.get(
    ["isRunning", "isWorking", "blockedSites"],
    (result: StorageData) => {
      if (result.isRunning) {
        startBackgroundTimer();
        // 如果是專心時間，啟用網站封鎖
...
chrome.storage.onChanged.addListener((changes, namespace) => {
  if (namespace === "local") {
    ...
    if (changes.isWorking || changes.blockedSites || changes.isRunning) {
      chrome.storage.local.get(
        ["isWorking", "blockedSites", "isRunning"],
        (result: StorageData) => {
          // 只在專心時間且計時器運行時封鎖
